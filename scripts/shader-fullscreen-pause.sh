...
pause_services() {
    ...
    if pgrep -x "python" > /dev/null; then
        WALLPAPER_PID=$(pgrep -x "python" | tr '\n' ' ')
        for pid in $WALLPAPER_PID; do
            kill -STOP "$pid" &
        done
    fi
}
...
is_window_fullscreen() {
    ...
    # Get the window's process name first
    ...
    if [ -n "$window_pid" ]; then
        ...
        # Special case: ignore ghostty fullscreen
        if [ "$process_name" = "ghostty" ]; then
            return 1  # Never pause for ghostty
        fi
    fi

    # For other applications, check if window has fullscreen state
